...
fn render_header_status(
    ...
    let (dot, dot_style) = if matches!(state, AgentState::Working) {
        (
            super::spinner_frame(app.visual_motion_tick()),
...
fn render_mobile_switcher_content(
    ...
    if !entries.is_empty() {
        ...
        for entry in &entries {
            ...
            let (icon, icon_style) =
                agent_icon(entry.state, entry.seen, app.visual_motion_tick(), p);
...
mod tests {
    ...
    async fn mobile_header_uses_live_root_runtime_cwd_for_workspace_label() {
        let unique = format!(
            "nagi-mobile-header-runtime-cwd-{}-{}",
            std::process::id(),
        ...
        assert!(row.contains("nagi"), "header row: {row:?}");
        assert!(

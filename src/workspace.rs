...
pub(crate) use self::git::git_worktree_info;
...
pub struct Workspace {
    ...
    /// Explicit Nagi-managed worktree grouping provenance.
...
mod tests {
    ...
    #[test]
    fn generated_workspace_ids_are_monotonic_base32_handles() {
        ...
        let first_number = public_workspace_number(&first).expect("valid first workspace handle");
        let second_number =
            public_workspace_number(&second).expect("valid second workspace handle");
    ...
    fn workspace_identity_follows_first_tab_root_pane_cwd() {
        ...
        terminals.insert(
            terminal_id.clone(),
            TerminalState::new(terminal_id, PathBuf::from("/nagi-test/pion")),
        );
        ...
        assert_eq!(
            ...
            Some(PathBuf::from("/nagi-test/pion"))
        );
    }

...
///
/// Contractual override behavior for auto-detect uses `NAGI_SOCKET_PATH`.
/// This variable is kept as a fallback for callers that explicitly need a
/// client-only override when `NAGI_SOCKET_PATH` is not set.
pub const CLIENT_SOCKET_PATH_ENV_VAR: &str = "NAGI_CLIENT_SOCKET_PATH";
...
/// 2. If `NAGI_SOCKET_PATH` is set, derive the client socket path from it by
///    inserting `-client` before `.sock` (e.g. `nagi.sock` -> `nagi-client.sock`).
///    This keeps JSON API and client socket overrides consistent.
/// 3. Otherwise, honor `NAGI_CLIENT_SOCKET_PATH` (legacy/testing fallback).
...
pub(crate) fn prepare_socket_path(path: &Path) -> io::Result<()> {
    crate::ipc::prepare_socket_path(path, |path| {
        format!(
            "nagi server is already running (socket busy at {})",
            path.display()
        )
    })
}
...
mod tests {
    ...
    fn client_socket_path_derived_from_api_socket_override() {
        let path = client_socket_path_from_overrides(Some("/tmp/test-nagi.sock"), None);
        assert_eq!(path, PathBuf::from("/tmp/test-nagi-client.sock"));
    }
    ...
    fn client_socket_path_respects_legacy_client_override_without_api_override() {
        let path = client_socket_path_from_overrides(None, Some("/tmp/test-nagi-client.sock"));
        assert_eq!(path, PathBuf::from("/tmp/test-nagi-client.sock"));
    }
    ...
    fn client_socket_path_defaults_to_config_dir() {
        ...
        assert_eq!(path, crate::config::config_dir().join("nagi-client.sock"));
    }

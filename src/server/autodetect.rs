//! Auto-detect launch behavior for the `nagi` command.
//!
//! When the user runs `nagi` with no subcommand:
...
/// directory where the user ran `nagi`.
pub(crate) const STARTUP_CWD_ENV_VAR: &str = "NAGI_STARTUP_CWD";

// ---------------------------------------------------------------------------
// Server detection
// ---------------------------------------------------------------------------

/// Checks whether a nagi server is currently listening on the client socket.
///
...
/// Checks whether a nagi server is listening at a specific socket path.
...
fn validate_running_server_compatibility() -> io::Result<()> {
    let Some(status) = read_server_status()? else {
        return Err(io::Error::other(format!(
            "a nagi server is listening, but its status API is unavailable.\n\n{}\nIf that fails, stop the old server process manually.",
    ...
    Err(io::Error::other(format!(
        "Nagi was updated, but this session is still running the old server.\n\nserver: v{} protocol {}\nclient: v{} protocol {}\n\n{}",
...
/// Spawns the nagi server as a background daemon process.
///
...
/// - Inherits relevant environment variables (`XDG_CONFIG_HOME`, `NAGI_SESSION`,
...
pub fn spawn_server_daemon() -> io::Result<u32> {
    let exe = std::env::current_exe().map_err(|err| {
        io::Error::new(
            err.kind(),
            format!("failed to determine nagi executable path: {err}"),
        )
    })?;
    ...
    let child = command.spawn().map_err(|err: io::Error| {
        io::Error::new(err.kind(), format!("failed to spawn nagi server: {err}"))
    })?;
...
pub fn wait_for_server_socket(socket_path: &Path, timeout: Duration) -> io::Result<()> {
    ...
    Err(io::Error::new(
        io::ErrorKind::TimedOut,
        format!(
            "server did not become ready within {}s (socket: {}). The background server may still be starting; try `nagi` again, or check {}",
            timeout.as_secs(),
            socket_path.display(),
...
///
/// This is the entry point called from `main.rs` when the user runs `nagi`
...
mod tests {
    ...
    fn server_daemon_command_clears_socket_overrides_for_explicit_session() {
        ...
        std::env::set_var("NAGI_CLIENT_SOCKET_PATH", "/tmp/inherited-client.sock");
        ...
        let command = build_server_daemon_command(PathBuf::from("/tmp/nagi-test"));
        ...
        assert!(envs.iter().any(|(key, value)| {
            *key == OsStr::new("NAGI_CLIENT_SOCKET_PATH") && value.is_none()
        }));
        std::env::remove_var(crate::api::SOCKET_PATH_ENV_VAR);
        std::env::remove_var("NAGI_CLIENT_SOCKET_PATH");
        std::env::remove_var(crate::session::SESSION_ENV_VAR);
        crate::session::clear_explicit_session_for_test();
    }
...
"#,
    ...
    fn validate_running_server_compatibility_names_session_commands_for_protocol_mismatch() {
        ...
        assert!(
            message.contains("Run `nagi session stop work`"),
            "unexpected error: {message}"
        );
        assert!(
            message.contains("then run `nagi session attach work` again"),
            "unexpected error: {message}"
        );

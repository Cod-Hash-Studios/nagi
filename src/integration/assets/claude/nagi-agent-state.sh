...
action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/nagi-claude-hook.XXXXXX")" || exit 0
...
source = "nagi:claude"
action = os.environ.get("NAGI_ACTION", "")
...
if hook_event_name == "SubagentStop":
    # SubagentStop is a completion event. Older Nagi integrations mapped it

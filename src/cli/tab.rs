...
fn tab_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: nagi tab get <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: nagi tab get <tab_id>");
...
fn tab_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: nagi tab focus <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: nagi tab focus <tab_id>");
...
fn tab_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: nagi tab rename <tab_id> <label>");
        return Ok(2);
    }
...
fn tab_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: nagi tab close <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: nagi tab close <tab_id>");
...
fn print_tab_help() {
    eprintln!("nagi tab commands:");
    eprintln!("  nagi tab list [--workspace <workspace_id>]");
    eprintln!(
        "  nagi tab create [--workspace <workspace_id>] [--cwd PATH] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]"
    );
    eprintln!("  nagi tab get <tab_id>");
    eprintln!("  nagi tab focus <tab_id>");
    eprintln!("  nagi tab rename <tab_id> <label>");
    eprintln!("  nagi tab close <tab_id>");
}

...
pub(super) fn command() -> Command {
    let command = Command::new("nagi")
        ...
        .arg(option("remote", "TARGET").help("Attach through SSH to a remote Nagi server"))
        .arg(
...
mod tests {
    ...
    fn zsh_completion_contains_public_commands_and_values() {
        ...
        clap_complete::generate(clap_complete::Shell::Zsh, &mut cmd, "nagi", &mut output);
        let script = String::from_utf8(output).unwrap();
        assert!(script.contains("#compdef nagi"));

...
fn parse_status_args(args: &[String]) -> Option<(StatusScope, bool)> {
    match args.first().map(|arg| arg.as_str()) {
        ...
        Some("server") => {
            parse_status_scope_args(args, StatusScope::Server, "nagi status server [--json]")
        }
        Some("client") => {
            parse_status_scope_args(args, StatusScope::Client, "nagi status client [--json]")
        }
        Some("help" | "--help" | "-h") => {
            if args.len() > 1 {
                print_status_help();
...
fn print_status_help() {
    eprintln!("nagi status commands:");
    eprintln!("  nagi status [--json]         show local client and running server status");
    eprintln!("  nagi status server [--json]  show running server status");
    eprintln!("  nagi status client [--json]  show local client binary status");
}

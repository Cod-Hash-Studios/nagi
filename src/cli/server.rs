...
fn server_stop(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: nagi server stop");
        return Ok(2);
    }
...
fn server_reload_config(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: nagi server reload-config");
        return Ok(2);
    }
...
fn server_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        _ => {
            eprintln!("usage: nagi server agent-manifests [--json]");
            return Ok(2);
        }
    };
...
fn server_reload_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: nagi server reload-agent-manifests");
        return Ok(2);
    }
...
fn server_update_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        [flag] if flag == "--json" => true,
        _ => {
            eprintln!("usage: nagi server update-agent-manifests [--json]");
            return Ok(2);
        }
    };
...
fn server_live_handoff(args: &[String]) -> std::io::Result<i32> {
    let Some(params) = parse_live_handoff_params(args) else {
        eprintln!(
            "usage: nagi server live-handoff [--import-exe <path>] [--expected-protocol <n>] [--expected-version <version>]"
        );
        return Ok(2);
    };
...
fn print_server_help() {
    eprintln!("nagi server commands:");
    eprintln!("  nagi server                run as headless server");
    eprintln!("  nagi server stop           stop the running server via the API socket");
    eprintln!("  nagi server live-handoff   hand off live panes to a new local server");
    eprintln!("  nagi server reload-config  reload config.toml in the running server");
    eprintln!("  nagi server agent-manifests [--json]  show agent detection manifest status");
    eprintln!(
        "  nagi server update-agent-manifests [--json]  fetch and reload agent detection manifests"
    );
    eprintln!("  nagi server reload-agent-manifests  reload agent detection manifests in the running server");
}
...
mod tests {
    ...
    fn live_handoff_params_parse_remote_update_fields() {
        let args = vec![
            ...
            "/home/me/.local/bin/nagi".to_string(),

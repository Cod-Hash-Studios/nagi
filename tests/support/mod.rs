...
const RUNTIME_OWNER_MARKER: &str = ".nagi-test-owner-pid";
pub const CURRENT_PROTOCOL: u32 = 16;

pub fn register_spawned_nagi_pid(pid: Option<u32>) {
...
pub fn unregister_spawned_nagi_pid(pid: Option<u32>) {
    let Some(pid) = pid else {
        return;
    };
...
#[cfg(target_os = "linux")]
pub fn nagi_server_pids_for_runtime_dir(runtime_dir: &Path) -> std::io::Result<Vec<u32>> {
...
pub fn cleanup_registered_nagi_pids() {
    let pids: Vec<u32> = {
        let mut registry = pid_registry_lock();
...
fn start_global_watchdog() {
    thread::spawn(|| loop {
        ...
        if let Err(err) = cleanup_servers_with_missing_runtime_dir() {
            eprintln!("nagi test cleanup watchdog error: {err}");
        }
    });
}
...
fn iter_worktree_server_pids() -> std::io::Result<Vec<u32>> {
    ...
    for entry in proc_entries {
        ...
        if is_test_nagi_server_process(pid) {
            pids.push(pid);
        }
    }
...
fn is_test_nagi_server_process(pid: u32) -> bool {
    let Some(exe_path) = proc_link_target(pid, "exe") else {
        return false;
    };

    if !is_test_nagi_binary(&exe_path) {
        return false;
    }
...
fn process_runtime_dir(pid: u32) -> std::io::Result<Option<PathBuf>> {
    ...
    for entry in environ.split(|byte| *byte == 0) {
        ...
        if let Some(value) = kv.strip_prefix("NAGI_SOCKET_PATH=") {
...
fn is_test_nagi_binary(path: &Path) -> bool {
    path.ends_with("target/debug/nagi") && path.starts_with(current_checkout_root())
}
...
mod tests {
    ...
    fn unique_missing_runtime_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "nagi-watchdog-scoping-{label}-{}-{unique}",
            std::process::id()
        ))
    }
    ...
    fn test_binary_matcher_accepts_current_checkout_debug_binary() {
        let binary = current_checkout_root().join("target/debug/nagi");
        assert!(
            is_test_nagi_binary(&binary),
    ...
    fn test_binary_matcher_rejects_installed_binary() {
        assert!(
            !is_test_nagi_binary(Path::new("/home/can/.local/bin/nagi")),

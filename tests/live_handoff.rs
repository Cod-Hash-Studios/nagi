...
use support::{
    cleanup_test_base, client_handshake, register_runtime_dir, register_spawned_nagi_pid,
    send_input, unregister_spawned_nagi_pid, wait_for_disconnect, wait_for_socket,
};
...
fn spawn_server(config_home: &Path, runtime_dir: &Path, api_socket: &Path) -> SpawnedNagi {
...
fn spawn_server_with_env(
    ...
    cmd.env(
        "NAGI_CLIENT_SOCKET_PATH",
        runtime_dir.join("nagi-client.sock"),
    );
...
fn spawn_named_session_server(
    ...
    cmd.env("XDG_CONFIG_HOME", config_home);
    cmd.env("XDG_RUNTIME_DIR", runtime_dir);
    cmd.env("NAGI_SESSION", session_name);
...
fn spawn_default_session_server(config_home: &Path, runtime_dir: &Path) -> SpawnedNagi {
    fs::create_dir_all(config_home.join("nagi-dev")).unwrap();
    fs::create_dir_all(runtime_dir).unwrap();
    fs::write(
        config_home.join("nagi-dev/config.toml"),
...
fn spawn_server_with_args_and_socket_env(
    ...
    if let Some(api_socket_env) = api_socket_env {
        cmd.env("NAGI_SOCKET_PATH", api_socket_env);
    } else {
        cmd.env_remove("NAGI_SOCKET_PATH");
    }
    if let Some(client_socket_env) = client_socket_env {
        cmd.env("NAGI_CLIENT_SOCKET_PATH", client_socket_env);
    } else {
        cmd.env_remove("NAGI_CLIENT_SOCKET_PATH");
    }
    cmd.env("SHELL", "/bin/sh");
...
fn wait_for_replacement_server_pid(runtime_dir: &Path, old_pid: u32, timeout: Duration) -> u32 {
    ...
    while Instant::now() < deadline {
        last_pids = support::nagi_server_pids_for_runtime_dir(runtime_dir).unwrap_or_default();
...
fn wait_for_replacement_server_pid(_runtime_dir: &Path, old_pid: u32, timeout: Duration) -> u32 {
    let handoff_socket_pattern = format!("nagi-handoff-{old_pid}.sock");
    let deadline = Instant::now() + timeout;
...
fn live_handoff_preserves_named_session_socket_paths() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let session_dir = config_home.join("nagi-dev/sessions/work");
    let api_socket = session_dir.join("nagi.sock");
    let client_socket = session_dir.join("nagi-client.sock");
    ...
    assert!(
        !config_home.join("nagi-dev/nagi.sock").exists(),
...
fn live_handoff_ignores_leaked_default_socket_env_for_named_session() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let default_session_dir = config_home.join("nagi-dev");
    let default_api_socket = default_session_dir.join("nagi.sock");
    let default_client_socket = default_session_dir.join("nagi-client.sock");
    let work_session_dir = config_home.join("nagi-dev/sessions/work");
    let work_api_socket = work_session_dir.join("nagi.sock");
    let work_client_socket = work_session_dir.join("nagi-client.sock");
...
fn live_handoff_preserves_client_socket_env_without_api_socket_env() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let api_socket = config_home.join("nagi-dev/nagi.sock");
...
fn live_handoff_accepts_canonical_pane_id_from_child_env() {
    ...
    assert_ok(request(
        ...
        serde_json::json!({
            ...
            "method": "pane.send_input",
            "params": {"pane_id": pane_id, "text": format!("printf '%s' \"$NAGI_PANE_ID\" > {}", pane_id_marker.display()), "keys": ["Enter"]}
        }),
    ));
...
fn live_handoff_preserves_http_servers_across_multiple_sessions() {
    ...
    let runtime_dir = base.join("runtime");
    let sessions = [
        (None, config_home.join("nagi-dev/nagi.sock")),
        (
            Some("work"),
            config_home.join("nagi-dev/sessions/work/nagi.sock"),
        ),
    ];
...
fn live_handoff_import_failure_rolls_back_old_server_at(failure_point: &str) {
    ...
    let spawned = spawn_server_with_env(
        ...
        &runtime_dir,
        &api_socket,
        &[("NAGI_TEST_HANDOFF_IMPORT_FAIL", failure_point)],
    );
    wait_for_socket(&api_socket, Duration::from_secs(10));
    register_runtime_dir(&runtime_dir);

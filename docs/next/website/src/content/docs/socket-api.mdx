...
| `mission.start` | `mission_id`, `run_id`, `provider`, `mode`; optional `worktree_path` | Claims one canonical Git checkout and starts Codex or Claude Code in managed, read-only mode. The closure plan must already exist. |
...
`provider` accepts `codex`, `claude_code`, or `open_code`; `mode` accepts
`managed` or `passthrough`. In the current build, OpenCode mission start and
all passthrough starts return an availability error. Managed Codex and Claude
Code processes receive read-only workspace access. There is no API switch that
...
| `provider_unavailable`, `provider_mode_unavailable` | OpenCode start or passthrough mode is not wired in the current build. |

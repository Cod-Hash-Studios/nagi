---
title: Socket API
description: Control a running Nagi server from scripts, tools, and coding agents.
---
...
| Layer | Use it for |
| --- | --- |
| Agent skill | Teaching a coding agent how to use Nagi from inside a pane. |
...
The installed CLI can print the socket protocol schema bundled with that Nagi
binary:
...
Plain `nagi api schema` prints a short summary. `--json` prints the full JSON
...
- create, configure, inspect, and start durable read-only missions
...
| Mission | `mission.create`, `mission.list`, `mission.get`, `mission.configure`, `mission.start`, `mission.respond` |
...
Missions are durable, server-owned task records. A mission binds an objective
and acceptance criteria to one Git repository, an immutable closure plan, one
run, and any unresolved attention items. Mission state survives a server
restart.

The raw API flow is:

1. `mission.create` records the objective and acceptance criteria.
2. `mission.configure` maps every acceptance criterion to required checks.
3. `mission.start` claims a checkout and starts the managed provider.
4. `mission.get` or `mission.list` observes the durable state.
5. `mission.respond` is present but intentionally refuses public provider responses.

### Create, list, and inspect

`mission.create` requires:

| Field | Contract |
| --- | --- |
| `mission_id` | 1-128 ASCII letters, digits, `_`, `.`, `:`, or `-`. |
| `title` | 1-256 characters. |
| `repository_path` | Absolute path to the root of an existing Git checkout. |
| `objective` | 1-8192 characters. |
| `acceptance_criteria` | 1-16 non-empty entries, each at most 1024 characters. Order is significant. |
...
The response has `type: "mission_created"`, a full `mission`, and `created`.
Repeating the exact request is safe and returns `created: false`. Reusing the id
with a different definition returns `mission_conflict`.

`mission.list` takes `{}` and returns `type: "mission_list"`. Each summary
contains `mission_id`, `title`, `repository_path`, `status`,
`unresolved_attention_count`, and `updated_at_millis`; it deliberately omits
the objective, criteria, and run details. Use `mission.get` for the full record:
...
A full `MissionInfo` contains the definition plus `closure_configured`,
`check_count`, `status`, optional `run`, `unresolved_attention_count`, and
`updated_at_millis`. Status values are `draft`, `preparing`, `active`,
`review_required`, `ready_to_close`, `blocked`, `failed`, and `archived`. When
present, `run` contains `run_id`, `provider`, `mode`, `worktree_path`, and
`base_revision`. Native provider session ids are intentionally not exposed.

### Configure the closure plan

`mission.configure` accepts 1-32 command or manual checks. `covers` contains
zero-based indexes into the mission's `acceptance_criteria`. Check ids must be
unique, at least one check must be required, and the required checks must
collectively cover every criterion. Configure the plan before starting a run.
...
Command checks use an executable `program` and argv `args`, not a shell command
string. `cwd`, artifact paths, and `exact`/`prefix` rules are relative to the
claimed checkout and cannot be absolute or contain `..`. Path prefixes are
component-aware: `src` matches `src/auth.rs`, not `src2/auth.rs`.
`relevant_paths: []` and `{"type":"all"}` both select the whole checkout.
Ignored files are excluded unless `include_ignored` is true. `required_artifacts`
lists files that must be present in the verified workspace.

A manual check has this shape:
...
Manual checks require at least one reviewer id. `allow_override` defaults to
false. `include_ignored` and `allow_override` are the only optional check
fields; send the other fields even when their arrays are empty.

The response has `type: "mission_configured"`, a full `mission`, and
`configured`. Repeating the same check set returns `configured: false`.
Configuration is immutable: a different check set returns `mission_conflict`.
...
`worktree_path` is optional and defaults to `repository_path`. It must identify
a checkout of the mission repository. Nagi takes an exclusive mission lease
for that checkout, so two mission writers cannot use it concurrently. The
checkout's current Git `HEAD` becomes the run's immutable `base_revision`.

Every socket API start is read-only. Codex uses its read-only sandbox. Claude
Code uses a hermetic read-only provider profile: permission mode `plan`, safe
mode, a strict empty MCP configuration, and only `Read`, `Glob`, `Grep`, and
`AskUserQuestion` tools. This excludes write, shell, and externally configured
MCP tools from that Claude process.

`mission.start` has no write-consent boolean, token, or equivalent JSON field,
and rejects unknown fields. The socket API cannot elevate a run to write
access. A future write flow is reserved for direct interactive confirmation in
the TUI mission cockpit; it is not a currently available public capability.
Recovered runs also resume read-only.

The current build supports `provider: "codex"` and `provider: "claude_code"`
with `mode: "managed"`. The wire enum reserves `open_code`, and the mode enum
reserves `passthrough`, but those combinations currently return an availability
error. Do not use a reserved value as feature detection.

The immediate response has `type: "mission_run_started"` and a full mission in
`preparing` state. Provider readiness moves it to `active` asynchronously. A
completed provider turn moves it to `review_required`, not directly to
`ready_to_close`; provider self-reporting is not proof of acceptance.

### Public responses require interactive consent

`mission.respond` keeps a typed request shape in the socket schema:
...
For every well-formed public call, the server returns
`interactive_consent_required`. It does not inspect the requested decision for
authorization, record a response attempt, send a provider command, or resolve
the attention item. `mission.list` and `mission.get` expose only the number of
unresolved attention items, not provider prompts or response tokens.

There is no socket API boolean, bearer token, or alternate JSON payload that
can bypass this guard. Provider approvals and answers are reserved for a future
interactive confirmation flow in the TUI mission cockpit; that flow is not a
currently available public capability.

### Mission errors

In addition to `invalid_request` for malformed JSON or missing/wrongly typed
fields, mission methods can return:

| Code | Meaning |
| --- | --- |
| `invalid_mission` | A mission id, text field, repository field, or criteria set is invalid. |
| `invalid_repository` | The repository/worktree is unavailable, is not Git, or is not the required checkout root. |
| `mission_not_found` | The target mission does not exist. |
| `mission_conflict` | An idempotent create/configure call conflicts with the durable definition. |
| `invalid_closure` | Checks are invalid, do not cover every criterion, or were not configured before start. |
| `provider_unavailable` | The requested provider is unavailable. |
| `provider_mode_unavailable` | The requested provider mode is not implemented. |
| `run_conflict` | The run id is already active in the server. |
| `invalid_run` | The run id cannot be used for its durable worktree claim. |
| `provider_start_failed` | The provider process or its initial command could not start. |
| `interactive_consent_required` | Public `mission.respond` is guarded and has produced no effect; an interactive cockpit confirmation would be required. |
| `mission_runtime_error` | A durable state, ownership, worktree lease, or journal invariant rejected the operation. |

Treat unknown codes as non-success and preserve the server's `message` for
diagnostics. Do not retry `mission.respond` with invented consent fields or
tokens; the public method is intentionally non-authorizing.
...
From the CLI, `nagi api snapshot` prints the live `session.snapshot` response
...
`pane.send_keys` and `pane.send_input.keys` accept Nagi key-combo strings:
...
Nagi returns that pane. When it is omitted, Nagi returns the active focused
pane.
...
Nagi replies with `ok`, send one JSON header and then exactly `data_length` raw
...
provided, Nagi creates the replacement tab first and then closes the old tab.
...
Process-launching methods accept an `env` object. Nagi applies those key/value
pairs to the newly launched process only. Nagi also injects `NAGI_SOCKET_PATH`,
`NAGI_ENV=1`, `NAGI_WORKSPACE_ID`, `NAGI_TAB_ID`, and `NAGI_PANE_ID` into
managed pane processes. Nagi-managed variables are authoritative when they
...
`pane.moved`; Nagi does not emit fake pane close/create events for the moved
terminal process.
...
`client.window_title.clear` restores Nagi's default title. The response is
...
is a package with a `nagi-plugin.toml` manifest. The manifest declares
...
Installed and linked plugins persist across restarts. Nagi writes a
`plugins.json` registry file alongside `session.json` on `plugin.link`,
`plugin.unlink`, `plugin.enable`, and `plugin.disable`. The
`nagi plugin install` CLI also writes the same registry when Nagi is not
running, then startup loads it automatically. On startup, Nagi re-reads each
...
The path can be a plugin directory containing `nagi-plugin.toml` or a direct
...
`min_nagi_version` is required. The server refuses to link a plugin when the
field is missing, invalid, or newer than the running Nagi binary.
...
command, and returns the Nagi-built invocation context plus the started command
...
Nagi injects `NAGI_SOCKET_PATH`, `NAGI_BIN_PATH`, `NAGI_ENV=1`,
`NAGI_PLUGIN_ID`, `NAGI_PLUGIN_ROOT`, `NAGI_PLUGIN_CONFIG_DIR`,
`NAGI_PLUGIN_STATE_DIR`, `NAGI_PLUGIN_CONTEXT_JSON`, and available
`NAGI_WORKSPACE_ID`, `NAGI_TAB_ID`, and `NAGI_PANE_ID` values. Action
commands also receive `NAGI_PLUGIN_ACTION_ID`; event hooks receive
`NAGI_PLUGIN_EVENT` and `NAGI_PLUGIN_EVENT_JSON`; pane commands receive
...
Event hooks run for enabled installed plugins when Nagi emits a matching event
name such as `worktree.created`.

There is no Nagi-managed plugin storage API in v1. `NAGI_PLUGIN_CONFIG_DIR`
and `NAGI_PLUGIN_STATE_DIR` are path discovery only; plugins own their files,
...
`NAGI_PANE_ID` to its process. Popup launch returns `ok`; `popup.close` closes
...
behave like normal Nagi panes, and `plugin.pane.focus` and `plugin.pane.close`
continue to operate on those panes.

## Socket transport

Nagi uses newline-delimited JSON over a local socket. On Unix, that socket is a
...
For plugins, prefer invoking `NAGI_BIN_PATH` and the CLI wrappers when you need
...
`workspace_id`, and the current `scroll` metrics whenever Nagi observes a

...
GitHub トピック `nagi-plugin` が付いた公開 GitHub リポジトリの自動インデックスです。
プラグインは普通の GitHub リポジトリのままです: `nagi-plugin.toml` を含めて公開し、
`nagi plugin install owner/repo[/subdir]` を共有してください。

プラグインを掲載するには、公開リポジトリに GitHub トピック `nagi-plugin` を

...
`worktree_path` は省略可能で、既定値は `repository_path` です。ミッションのリポジトリに属するチェックアウトを指定する必要があります。Herdr はそのチェックアウトに排他的なミッションリースを取得するため、2 つのミッションライターが同時に利用することはできません。チェックアウトの現在の Git `HEAD` が、実行の変更不能な `base_revision` になります。

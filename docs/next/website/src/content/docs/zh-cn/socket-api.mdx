...
| `mission.start` | `mission_id`、`run_id`、`provider`、`mode`;可选 `worktree_path` | 独占一个规范 Git checkout,以托管只读模式启动 Codex 或 Claude Code。必须先配置 closure plan。 |
...
`provider` 接受 `codex`、`claude_code` 或 `open_code`;`mode` 接受 `managed`
或 `passthrough`。当前构建中,OpenCode 任务启动和所有 passthrough 启动都会返回
不可用错误。托管 Codex 和 Claude Code 进程只获得工作区读取权限。

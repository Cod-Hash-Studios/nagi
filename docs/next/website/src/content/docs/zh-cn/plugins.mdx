---
title: 插件
description: 编写带有清单动作、事件钩子和窗格的本地 Nagi 插件。
---

Nagi 插件是可分享、可执行的工作流包。插件可以是 Bash 脚本、JavaScript
...
没有单独的插件 SDK,也没有受限的命令集。整个 Nagi CLI 就是插件 API:
...
清单,以及它运行的脚本或二进制。`nagi plugin install` 在交互式终端中会
展示来源和将要运行的命令的预览,你可以在确认前审查。对已经信任的来源用
`--yes`,想固定某个特定版本时用 `--ref`。

Nagi 校验清单,并把每个插件的配置和状态放在各自的目录中,但它不会审查
或沙箱化插件的行为。第三方插件来自它们的作者,而不是 Nagi,审查与运行
由你自行决定。
...
社区插件可以在[插件市场](/plugins/)中发现,它是打了 `nagi-plugin` 主题
标签的公开 GitHub 仓库的自动索引。插件仍然是普通的 GitHub 仓库: 发布一个
带 `nagi-plugin.toml` 的仓库,然后分享
`nagi plugin install owner/repo[/subdir]`。

要让插件被收录,给它的公开仓库添加 GitHub 主题标签 `nagi-plugin`。索引

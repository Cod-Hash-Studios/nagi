---
title: 会话状态与恢复
description: 理解 Nagi 保持哪些实时状态、重启后恢复什么、从历史回放什么、通过智能体集成恢复什么会话,以及更新时如何交接。
---

Nagi 有多条状态路径,它们解决的是不同的问题。
...
普通分离会让 Nagi 服务器继续运行。窗格、shell、智能体、服务器、测试和命令进程都在该服务器中继续运行。
...
如果 Nagi 服务器停止后再启动,原来的窗格进程已经不在了。Nagi 会恢复保存的会话形态: 工作区、标签页、窗格、cwd、布局和焦点。

快照恢复不会保留运行中的 shell、服务器、测试或任意进程。无法使用更强恢复路径的窗格,会在各自保存的目录中作为新 shell 回来。

## 窗格屏幕历史回放

窗格屏幕历史在服务器完全重启后恢复最近的终端内容。它恢复的是 Nagi 能展示的内容,而不是原来的进程。
...
开启后,Nagi 把保存的窗格历史存放在 `session.json` 旁边的 `session-history.json` 中。请像对待终端历史一样对待 Nagi 的配置/会话目录。

## 智能体原生会话恢复

一些智能体可以恢复它们自己的对话会话。Nagi 可以使用官方集成上报的会话引用,在 Nagi 服务器重启后重新启动受支持的智能体窗格。
...
Nagi 只会恢复那些通过当前官方 Nagi 集成上报了原生会话引用的窗格。

当客户端连接并提供终端尺寸和主题上下文后,Nagi 会跨工作区和标签页恢复符合条件的智能体窗格,不需要等每个窗格被聚焦。

智能体原生会话恢复需要以下版本或更新的 Nagi 集成:

| 智能体 | 最低 Nagi 集成版本 | 恢复命令 |
| --- | --- | --- |
...
运行 `nagi integration status` 查看已安装的集成版本。用 `nagi integration install <agent>` 重新安装过期的集成。

不受支持、缺失、无效、重复或过期的会话引用,会在保存的窗格目录中作为普通 shell 恢复。

如果某个窗格适用智能体原生会话恢复,Nagi 会对该窗格恢复智能体会话,而不是回放保存的窗格历史。

## 实时交接

实时交接用于需要替换运行中 Nagi 服务器的更新和远程连接流程。它请求旧服务器把实时窗格转移给新服务器,让窗格进程跨服务器替换继续运行。
...
普通的 `nagi update` 和普通的 `nagi --remote workbox` 默认使用常规的重启/停止流程。

`nagi update --handoff` 仍保留在继承的运行时中，但在 Nagi 发布签名产物之前，自动更新渠道保持禁用。请重新构建已审核的源码并重启会话。

---
title: Marketplace
description: Discover community Nagi plugins on GitHub, and get your own plugin listed.
---

The Nagi plugin marketplace is a discoverable index of community plugins.
Browse it at [Nagi plugin registry](/plugins/). It is an automatic index of public
GitHub repositories, not a reviewed catalog.

## Browse plugins

The [marketplace](/plugins/) lists every public repository tagged with the
GitHub topic `nagi-plugin`. Search by name, owner, description, or language,
...
itself, not that Nagi vetted it, so the
...
Publish a normal public GitHub repository with a `nagi-plugin.toml` manifest at
...
Add the GitHub topic `nagi-plugin` to a public repository. That topic is the
...
The index does not parse `nagi-plugin.toml` yet, so manifest fields like the
plugin `id`, declared `platforms`, and `min_nagi_version` are not shown in v1.

...
- イベントの購読と、出力や状態変化の待機
- 永続的なコーディングエージェントミッションの作成、確認、設定、開始
...
### ミッションメソッド: Early Access 契約

ミッション API は現在、意図と所有権を永続化しますが、管理プロバイダーとの境界は
意図的に限定されています:

| メソッド | 必須パラメータ | 現在の動作 |
| --- | --- | --- |
| `mission.create` | `mission_id`、`title`、絶対パスの `repository_path`、`objective`、`acceptance_criteria` | 永続的なドラフトを作成します。ID は 1–128 文字、空でない基準は 1–16 件です。同一リクエストの再実行は冪等で、内容を変えると `mission_conflict` になります。 |
| `mission.list` | なし | 永続化されたミッション概要を返します。 |
| `mission.get` | `mission_id` | 公開ミッション投影全体を返します。 |
| `mission.configure` | `mission_id`、`checks` | 1–32 件のコマンドまたは手動チェックを固定します。すべての受け入れ基準をカバーする必要があり、異なる再設定は拒否されます。 |
| `mission.start` | `mission_id`、`run_id`、`provider`、`mode`。`worktree_path` と `execute_declared_checks` は省略可能 | 正規の Git checkout を 1 つ占有し、Codex、Claude Code、OpenCode、または設定済み ACP エージェントを managed モードで開始します。事前に closure plan が必要です。 |
| `mission.respond` | `mission_id`、`run_id`、`attention_id`、`decision`。`answers` は省略可能 | 対話的な同意経路向けに予約されています。現在は常に `interactive_consent_required` を返し、プロバイダーへ応答を送りません。 |

`execute_project_recipe` は再起動時の復旧用に永続化されますが、公開 socket からは承認できません。
`true` を送ると `interactive_consent_required` になります。`.nagi/project.toml` の setup と
service 実行を承認できるのは、範囲を表示するローカル mission cockpit だけです。

`provider` は `codex`、`claude_code`、`open_code`、`acp`、`mode` は `managed` または
`passthrough` を受け付けます。すべての provider が managed 開始に対応します。ACP は
`providers.acp.command` に設定したローカル stdio コマンドを使い、ローカル cockpit で
明示的な書き込み範囲の確認が必要です。passthrough は利用不可エラーになります。公開
managed 開始には読み取り専用のワークスペースアクセスだけが与えられ、API から暗黙に
書き込み権限を付与することはできません。

| エラーコード | 意味 |
| --- | --- |
| `feature_unavailable` | このプラットフォームではミッションストレージを利用できません。 |
| `mission_not_found`、`mission_conflict` | ミッションがないか、不変の定義または設定が異なります。 |
| `invalid_mission`、`invalid_repository`、`invalid_closure` | ID、テキスト、Git root、基準の網羅、またはチェック宣言が不正です。 |
| `provider_unavailable`、`provider_mode_unavailable` | OpenCode 開始または passthrough モードは未配線です。 |
| `run_conflict`、`invalid_run`、`provider_start_failed` | run ID が競合している、claim identity を作れない、またはプロバイダープロセスを開始できません。 |
| `interactive_consent_required` | ミッション cockpit が判断を確認できるまで `mission.respond` はブロックされます。 |

入れ子の `checks`、パス規則、応答判断、結果オブジェクト、フィールド長の制約は、
操作対象と同じ Nagi バイナリに同梱された生成済みスキーマを正としてください。
...
```json
{"id":"req_plugin_link","method":"plugin.link","params":{"path":"/managed/plugin/nagi-plugin.toml","enabled":true,"source":{"kind":"github","owner":"owner","repo":"nagi-plugin","subdir":"worktree-bootstrap","requested_ref":"main","resolved_commit":"abc123","managed_path":"/data/plugins/github/<managed-checkout>","installed_unix_ms":1780000000000}}}
```

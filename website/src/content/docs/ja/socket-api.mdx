...
| `mission.start` | `mission_id`、`run_id`、`provider`、`mode`。`worktree_path` は省略可能 | 正規の Git checkout を 1 つ占有し、Codex または Claude Code を管理対象の読み取り専用モードで開始します。事前に closure plan が必要です。 |
...
`provider` は `codex`、`claude_code`、`open_code`、`mode` は `managed` または
`passthrough` を受け付けます。現在のビルドでは OpenCode のミッション開始とすべての
passthrough 開始は利用不可エラーになります。管理対象の Codex と Claude Code には
読み取り専用のワークスペースアクセスだけが与えられます。

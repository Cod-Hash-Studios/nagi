---
title: プラグイン
description: マニフェストのアクション、イベントフック、ペインを備えたローカル Nagi プラグインを作成します。
---

Nagi プラグインは、共有可能で実行可能なワークフローパッケージです。プラグインは
...
独立したプラグイン SDK や制限付きコマンドセットはありません。Nagi CLI 全体が
...
スクリプトやバイナリです。`nagi plugin install` は対話的なターミナルでソースと
実行されるコマンドのプレビューを表示するので、確定する前に確認できます。すでに
信頼しているソースには `--yes` を使い、特定のリビジョンが欲しいときは `--ref` で
固定してください。

Nagi はマニフェストを検証し、各プラグインの設定と状態を専用ディレクトリに
保ちますが、プラグインの動作をレビューしたりサンドボックス化したりはしません。
サードパーティのプラグインは Nagi ではなく作者のものです。検証と実行の判断は
あなた自身に委ねられます。

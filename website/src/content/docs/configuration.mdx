---
title: Configuration
description: Configure Nagi keybindings, themes, sidebar behavior, notifications, and advanced options.
---

Nagi works without a config file. Add one when you want custom keys, themes, sidebar layouts, notifications, or advanced behavior.
...
Nagi reads config from:

```text
Linux and macOS: ~/.config/nagi/config.toml
Windows:          %APPDATA%\nagi\config.toml
```

Run `nagi --help` to see the resolved config path for your system.
...
```bash
nagi --default-config > ~/.config/nagi/config.toml
```

If a config value is invalid, Nagi falls back to a safe default and shows a startup warning.

Nagi shows first-run setup when `onboarding` is missing or true. Continuing from onboarding writes `onboarding = false` and opens settings on the integrations tab. Set it when you want to skip that flow after setup.
...
You can also open the global menu in Nagi and choose `reload config`.

Reload applies most UI settings without restarting panes. Startup-only settings still need a restart.

## Terminal defaults

Set the executable Nagi uses for newly created interactive panes:
...
When unset or empty, Nagi uses `$SHELL`, then `/bin/sh` on Unix and PowerShell on Windows. This is an executable name or path, not a shell command line. Existing panes keep their current shell until they are recreated. Custom command keybinding strings run through `/bin/sh -c` for pane commands and `/bin/sh -lc` for detached commands on Unix; on Windows they run through `cmd.exe /d /c`.

Set how Nagi starts newly created interactive pane shells:
...
Nagi creates checkouts under `<directory>/<repo>/<branch-slug>`. For sibling-style checkouts, set this to a directory such as `~/Projects/nagi-worktrees`. Relative values are resolved to an absolute path when the app applies the config.

Worktree actions are available from Git workspace rows. `New worktree` creates a checkout, checks out an existing local branch when the entered branch already exists, otherwise creates the branch, opens it as a new Nagi workspace, and groups it under the source workspace. `Open worktree...` lists existing Git worktree checkouts for that repo; choosing an already-open checkout focuses it, and choosing a closed checkout opens it in the same group.

Grouped worktrees still behave like normal Nagi workspaces: they can be focused, renamed, closed, and contain their own tabs and panes. The parent row is the original workspace. Closing the parent row closes the whole Nagi group, but it does not delete checkout folders or branches.

Deleting a worktree checkout is explicit. Use `Delete worktree checkout...` on a grouped child workspace to run `git worktree remove`. Nagi first asks Git to remove safely. If Git refuses because the checkout has modified or untracked files, Nagi asks again before running the forced remove. Branches are not deleted.
...
When enabled, `nagi --remote` writes a private temporary SSH config that includes your `~/.ssh/config` and `/etc/ssh/ssh_config` first, then adds fallback `ServerAliveInterval` and `ServerAliveCountMax` values. Your own keepalive settings win. Nagi also uses a private per-attach OpenSSH control socket to reuse the first authenticated connection. Set `manage_ssh_config = false` to run remote attach through plain `ssh` without Nagi's generated config or control socket.

## Keybindings

For a guided introduction to the prefix and a vetted prefix-free setup, see [Keyboard](/docs/keyboard/).

Nagi has a prefix mode similar to tmux. The default prefix is `ctrl+b`. Keybinding strings are explicit: `prefix+n` means press the configured prefix and then `n`; `ctrl+alt+n` is a direct terminal-mode shortcut.
...
The default keymap is prefix-first so Nagi does not steal input from shells, editors, tmux, or terminal apps. Search `keys.` in the [Config reference](/docs/config-reference/) to see every action and default binding. The in-app help panel at `prefix+?` shows the active bindings.
...
If you have old custom keybindings and want the new defaults, run `nagi config reset-keys`. Nagi backs up `config.toml`, removes `[keys]` and `[[keys.command]]`, and uses built-in v2 defaults after restart or `nagi server reload-config`.
...
`NAGI_PANE_ID`; use `NAGI_ACTIVE_PANE_ID` for the underlying tiled pane.
...
Custom commands receive `NAGI_SOCKET_PATH`, `NAGI_BIN_PATH`, `NAGI_ACTIVE_WORKSPACE_ID`, `NAGI_ACTIVE_TAB_ID`, `NAGI_ACTIVE_PANE_ID`, and `NAGI_ACTIVE_PANE_CWD` when those values are available. Shell commands run from the focused pane's working directory when Nagi can detect it.

On Windows, custom command strings use `cmd.exe /d /c`, so environment variables use `%NAGI_BIN_PATH%` syntax. To run PowerShell syntax, invoke it explicitly, for example `powershell.exe -NoProfile -Command "..."`.
...
Search `theme.name` in the [Config reference](/docs/config-reference/) for every built-in theme. Use `terminal` when you want Nagi UI colors to follow your host terminal's ANSI palette.

To let Nagi switch its own UI theme when the host terminal reports a light/dark appearance change, enable theme auto-switching:
...
`auto_switch` defaults to `false`, so existing theme configs keep manual behavior. If `light_name` or `dark_name` is omitted, Nagi uses the matching built-in sibling for the configured `name` when one exists, such as `tokyo-night`/`tokyo-night-day` or `gruvbox`/`gruvbox-light`. Manual theme selection in Settings disables `auto_switch`.
...
A custom theme is a strict TOML file in the `themes/` directory beside
`config.toml`. Its filename is the theme id. For example,
`themes/quiet-focus.toml` is selected with `name = "quiet-focus"`.

Besides semantic colors, themes can change the cockpit's component behavior:

```toml
[components]
border = "plain"          # soft, rounded, plain, or ascii
selection = "fill"        # rail or fill
density = "compact"       # compact or comfortable
motion = "none"           # none or subtle
```

`density = "compact"` gives the cockpit more working area. `motion = "none"`
freezes spinners while keeping text state labels intact. These preferences
follow manual selection, live config reload, and automatic light/dark switching.
Invalid names, unknown fields, unsupported values, and low-contrast color
combinations fail closed while the last valid theme stays active.

Importing a Ghostty theme creates a safe color-only Nagi theme with conservative
component defaults:

```bash
nagi config import-ghostty-theme /path/to/ghostty-theme --name quiet-focus
```

## UI and sidebar

The sidebar is the main Nagi dashboard. Search `ui.` in the [Config reference](/docs/config-reference/) for sizing, collapsed mode, Agent panel ordering, mouse behavior, pane borders, and other presentation settings.
...
Tokens render in their configured order. Nagi normally separates adjacent values with ` · ` and uses a single space after `state_icon`. Missing values and their separators disappear; a row disappears when none of its tokens have a value. Each layout may contain at most 16 rows, with at most 16 tokens in each row.
...
Use `nagi workspace report-metadata` in the same way for custom Space tokens. Unreported custom tokens simply disappear. Metadata reporters provide values only; they cannot choose rows or styling. See [CLI reference: report metadata](/docs/cli-reference/#panes) for clearing, sequencing, and expiring values.

Sidebar row settings affect only the expanded desktop sidebar. Collapsed and mobile views keep their compact layouts.

## Notifications

Nagi can notify you when a background agent finishes or needs input:
...
Choose `nagi` for an in-app toast, `terminal` for an outer-terminal notification that works well over SSH, `system` for the local OS notification service, or `off` to disable popups. Nagi suppresses popups for the active tab. Search `ui.toast` in the [Config reference](/docs/config-reference/) for positions, delay behavior, and clipboard feedback settings.

## Sound

Sound notifications play through the local Nagi client. Custom sounds must be mp3 files; relative paths are resolved from the config file's directory.
...
Nagi resumes supported Agent conversations after a server restart by default:
...
On macOS, Nagi can temporarily switch to the system ASCII-capable input source while prefix commands and prefix-launched modes are active:
...
Nagi restores the previous input source when returning to terminal input or entering a text field. This setting has no effect on other platforms.
...
| `NAGI_LOG` | Set log filtering, for example `NAGI_LOG=nagi=debug`. |
| `NAGI_DISABLE_SOUND` | Disable sound playback even when `[ui.sound] enabled = true`. |
...
```text
~/.config/nagi/nagi.log
~/.config/nagi/nagi-client.log
~/.config/nagi/nagi-server.log
```

...
### 任务方法: Early Access 契约

任务 API 现在会持久保存意图和所有权,但托管 provider 的边界被有意限制:

| 方法 | 必填参数 | 当前行为 |
| --- | --- | --- |
| `mission.create` | `mission_id`、`title`、绝对路径 `repository_path`、`objective`、`acceptance_criteria` | 创建持久草稿。ID 长度为 1–128 个字符,非空验收标准为 1–16 条。重复完全相同的请求是幂等的,修改内容会返回 `mission_conflict`。 |
| `mission.list` | 无 | 返回持久化的任务摘要。 |
| `mission.get` | `mission_id` | 返回完整的公开任务投影。 |
| `mission.configure` | `mission_id`、`checks` | 固定 1–32 个命令或人工检查。每条验收标准都必须被覆盖,不同的第二次配置会被拒绝。 |
| `mission.start` | `mission_id`、`run_id`、`provider`、`mode`;可选 `worktree_path` 和 `execute_declared_checks` | 独占一个规范 Git checkout,以 managed 模式启动 Codex、Claude Code、OpenCode 或已配置的 ACP 智能体。必须先配置 closure plan。 |
| `mission.respond` | `mission_id`、`run_id`、`attention_id`、`decision`;可选 `answers` | 为交互式同意流程保留。当前总是返回 `interactive_consent_required`,不会向 provider 发送响应。 |

`execute_project_recipe` 会为重启恢复而持久化，但不能通过公共 socket 授权。传入 `true`
会返回 `interactive_consent_required`。只有显示 `.nagi/project.toml` 范围的本地 mission cockpit
可以批准 setup 和服务执行。

`provider` 接受 `codex`、`claude_code`、`open_code` 或 `acp`;`mode` 接受 `managed`
或 `passthrough`。所有 provider 都支持 managed 启动。ACP 使用
`providers.acp.command` 配置的本地 stdio 命令,并始终要求在本地驾驶舱中明确确认写入范围。
passthrough 会返回不可用错误。公共 managed 启动只获得工作区读取权限,API 不能暗中授予写入权限。

| 错误码 | 含义 |
| --- | --- |
| `feature_unavailable` | 当前平台不支持任务存储。 |
| `mission_not_found`、`mission_conflict` | 任务不存在,或不可变的定义/配置不一致。 |
| `invalid_mission`、`invalid_repository`、`invalid_closure` | ID、文本、Git root、验收标准覆盖或检查声明无效。 |
| `provider_unavailable`、`provider_mode_unavailable` | OpenCode 启动或 passthrough 模式尚未接通。 |
| `run_conflict`、`invalid_run`、`provider_start_failed` | run ID 冲突、无法形成 claim identity,或 provider 进程启动失败。 |
| `interactive_consent_required` | 在任务 cockpit 能确认决定之前,`mission.respond` 会被阻止。 |

嵌套的 `checks`、路径规则、响应决定、结果对象和字段长度限制,以被控制的
同一 Nagi 二进制中附带的生成 schema 为准。

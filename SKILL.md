---
name: nagi
description: "Control Nagi, a terminal multiplexer for coding agents. Use only when the user explicitly mentions Nagi or asks to use Nagi to inspect or control panes, tabs, workspaces, terminals, commands, or communication with another agent. Do not use merely because a task could benefit from a background terminal, delegation, or parallel work. Requires NAGI_ENV=1."
---

# Nagi

Nagi is a terminal multiplexer and runtime for coding agents. It organizes terminals into workspaces, tabs, and panes, detects agent identity and status, and exposes the running session through the `nagi` CLI.

Before issuing any control command, check that this agent is running inside a Nagi-managed pane:

```bash
test "${NAGI_ENV:-}" = 1
```

If the check fails, say that you are not running inside Nagi and stop. Do not inspect or control the focused Nagi session from outside Nagi.

When the check passes, the `nagi` binary in `PATH` talks to the running session. Use it to inspect neighboring work, create isolated terminal contexts, start agents and commands, read their output, and wait for state changes.
...
Do not run bare `nagi` for discovery; it launches or attaches the TUI. Do not probe a mutating nested command by omitting arguments; some commands, including `nagi workspace create`, are valid with defaults and will execute. Use the command-group output above instead.
...
Nagi injects the caller's stable context into every managed pane:

```bash
printf '%s\n' "$NAGI_WORKSPACE_ID" "$NAGI_TAB_ID" "$NAGI_PANE_ID"
```
...
```bash
nagi workspace list
nagi tab list --workspace "$NAGI_WORKSPACE_ID"
nagi pane current --current
nagi pane list --workspace "$NAGI_WORKSPACE_ID"
```
...
Use workspace and tab commands for organization. Use worktree commands only when you intentionally want Nagi to create, open, or remove a Git checkout.
...
```bash
nagi pane layout --pane "$NAGI_PANE_ID"
```
...
```bash
nagi pane split --current --direction right --no-focus
```
...
```bash
nagi pane rename <returned-pane-id> "reviewer"
nagi pane run <returned-pane-id> "codex"
```
...
```bash
nagi pane get <returned-pane-id>
nagi wait agent-status <returned-pane-id> --status idle --timeout 30000
nagi pane run <returned-pane-id> "Review the current diff and report only actionable findings."
```
...
```bash
nagi wait agent-status <returned-pane-id> --status working --timeout 30000
nagi wait agent-status <returned-pane-id> --status done --timeout 120000
nagi pane read <returned-pane-id> --source recent-unwrapped --lines 120
```
...
If a wait times out, inspect `nagi pane get <returned-pane-id>` and `pane read` before deciding what to do. A `blocked` agent needs input; an `unknown` pane may not yet contain a detected or integrated agent.

Submit follow-ups the same way:

```bash
nagi pane run <returned-pane-id> "Now check the failing test."
```
...
```bash
nagi pane run <returned-pane-id> "just test"
nagi wait output <returned-pane-id> --match "test result" --timeout 120000
nagi pane read <returned-pane-id> --source recent-unwrapped --lines 120
```
...
- Never run `nagi server stop` from an active session unless the user explicitly intends to stop the server and its pane processes.
- Never kill the main Nagi process. Use named test sessions for experiments that need an isolated server.

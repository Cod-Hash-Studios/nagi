...
NAGI_BIN="${NAGI_BIN:-$ROOT/target/debug/nagi}"
BASE="${BASE:-$(mktemp -d /tmp/nagi-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_nagi "$session" server stop >/dev/null 2>&1 || true
  done
...
run_nagi() {
  ...
  env -u NAGI_SOCKET_PATH \
    -u NAGI_CLIENT_SOCKET_PATH \
    -u NAGI_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$NAGI_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/nagi-dev' "$CONFIG_HOME"
  else
    printf '%s/nagi-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/nagi.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/nagi-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/nagi-dev/nagi.sock | "$CONFIG_HOME"/nagi-dev/sessions/*/nagi.sock)
      ;;
    *)
...
echo "using nagi: $NAGI_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/nagi-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/nagi-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_nagi "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
